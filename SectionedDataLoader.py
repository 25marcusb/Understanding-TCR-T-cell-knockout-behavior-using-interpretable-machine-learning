...
class LiveCellImageDataset(Dataset):
    def __init__(self,
        ...
        self.n_per_side = 600 // self.pixel_width  # Calculate the number of non-overlapping crops per side
        self.phase = phase
        self.red = red
    ...
    def __len__(self):
        return self.n_wells * self.n_per_side ** 2 * self.num_frames  # Total number of non-overlapping crops

    def __getitem__(self, idx):
        frames_per_well = self.n_per_side ** 2 * self.num_frames  # Calculate frames per well based on non-overlapping crops
        ...
        firstFrameNumber = self.start_frame + frame_within_sequence

        fname = self.fpath + 'cart_' + well_name + self.f_end_name
        dcl_ob = load_data_local(fname)

        x0 = (frame_position // self.n_per_side) * self.pixel_width  # Adjusted to calculate non-overlapping x0
        xf = x0 + self.pixel_width
        y0 = (frame_position % self.n_per_side) * self.pixel_width  # Adjusted to calculate non-overlapping y0
...
dataSet = LiveCellImageDataset(pixel_width=200, wells=['B3'])
